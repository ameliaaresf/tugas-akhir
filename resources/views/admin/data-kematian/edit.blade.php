...
<div class = "bg-gray-100 w-full mx-4 rounded-lg">
    <div class = "mx-4 my-4 text-center font-bold text-lg">
        <p>Form Data Kematian</p>
    </div>

    <form class="w-full mt-10 px-4 rounded-lg" action="{{route('admin.data-kematian.update', $datakematian->id)}}" method = "post">
    @method('PUT')  
    @csrf
      <div class="flex flex-wrap -mx-3 mb-6">
        <div class="w-full px-3 mb-6 md:mb-0 my-4">
          ...
          {{ $errors ->has('nik') ? 'is-invalid' : ''}}" id="nik" name = "nik" type="text" placeholder="Masukkan NIK" value="{{old('nik' , $datakematian->nik)}}">
          @if($errors->has('nik'))
  ...
  <div class="mb-3 xl:w-96 relative">
    ...
    @if($errors->has('id_penduduk'))
        ...
        <div class="w-full px-3 mb-6 md:mb-0 my-4">
          ...
          <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500
          {{ $errors ->has('tempat_lahir') ? 'is-invalid' : ''}}" id="tempat_lahir" name = "tempat_lahir" type="text" placeholder="Masukkan Tempat Lahir" value="{{old('tempat_lahir' , $datakematian->tempat_lahir)}}">
        ...
        <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="cus_name">Tanggal Lahir</label>
            <div class="relative">
                ...
                {{ $errors ->has('tgl_lahir') ? 'is-invalid' : ''}} " placeholder="Pilih Tanggal" value="{{old('tgl_lahir' , $datakematian->tgl_lahir)}}">
          ...
          <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500
          {{ $errors ->has('tempat_wafat') ? 'is-invalid' : ''}}" id="tempat_wafat" name = "tempat_wafat" type="text" placeholder="Masukkan Tempat wafat" value="{{old('tempat_wafat' , $datakematian->tempat_wafat)}}">
          @if($errors->has('tempat_wafat'))
          <div class="text-red-600 italic "> 
        ...
        <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="cus_name">Tanggal wafat</label>
            <div class="relative">
                ...
                <input id="tgl_wafat" datepicker="" datepicker-format="yyyy/mm/dd" type="text" name="tgl_wafat"  class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 
                {{ $errors ->has('tgl_wafat') ? 'is-invalid' : ''}} " placeholder="Pilih Tanggal" value="{{old('tgl_wafat' , $datakematian->tgl_wafat)}}">
                @if($errors->has('tgl_wafat'))
          <div class="text-red-600 italic">
          {{$errors->first('tgl_wafat')}} 
          </div>
          @endif
            </div>
          <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-last-name">
          ...
          <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500
          {{ $errors ->has('sebab_wafat') ? 'is-invalid' : ''}}" id="sebab_wafat" name = "sebab_wafat" type="text" placeholder="Masukkan Sebab wafat" value="{{old('sebab_wafat' , $datakematian->sebab_wafat)}}">
          @if($errors->has('sebab_wafat'))
          <div class="text-red-600 italic"> 
          {{$errors->first('sebab_wafat')}}
          </div>
          @endif
          <!-- <p class="text-red-500 text-xs italic">Please fill out this field.</p> -->
        </div>
  ...
  <div class="flex items-center mr-4 mb-4">
    <input id="laki-laki" type="radio" name="jk" class="hidden {{ $errors ->has('jk') ? 'is-invalid' : ''}}" value = "Laki-laki" {{ ($datakematian->jk=="Laki-laki")? "checked" : "" }} />
   ...
   <div class="flex items-center mr-4 mb-4">
    <input id="perempuan" type="radio" name="jk" class="hidden{{ $errors ->has('jk') ? 'is-invalid' : ''}}" value = "Perempuan" {{ ($datakematian->jk=="Perempuan")? "checked" : "" }} />
    <label for="perempuan" class="flex items-center cursor-pointer">
     ...
     @if($errors->has('jk'))
        ...
        <div class="w-full px-3 mb-6 md:mb-0 my-4">
          ...
          {{ $errors ->has('alamat') ? 'is-invalid' : ''}}" id="alamat" name = "alamat" type="text" placeholder="Masukkan Alamat" >{{old('alamat' , $datakematian->alamat)}}</textarea>
  ...
  <div class="mb-3 xl:w-96 relative">
    ...
    @if($errors->has('agama'))
        ...
        <div class="w-full px-3 mb-6 md:mb-0 my-4">
          ...
          <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500
          {{ $errors ->has('pekerjaan') ? 'is-invalid' : ''}}" id="pekerjaan" name = "pekerjaan" type="text" placeholder="Masukkan Pekerjaan" value="{{old('pekerjaan' , $datakematian->pekerjaan)}}">
        ...
        <div class="w-full px-3 mb-6 md:mb-0 my-4">
          ...
          <div class="flex items-center mr-4 mb-4">
    <input id="wni" type="radio" name="kw" class="hidden {{ $errors ->has('kw') ? 'is-invalid' : ''}}" value = "WNI" {{ ($datakematian->kw=="WNI")? "checked" : "" }} />
   ...
   <div class="flex items-center mr-4 mb-4">
    <input id="wna" type="radio" name="kw" class="hidden {{ $errors ->has('kw') ? 'is-invalid' : ''}}" value = "WNA" {{ ($datakematian->kw=="WNA")? "checked" : "" }} />
