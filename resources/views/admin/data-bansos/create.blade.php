...
<div class = "bg-gray-100 w-full mx-4 rounded-lg">
    ...
    <form class="w-full mt-10 px-4 rounded-lg" action="{{route('admin.data-bansos.store')}}" method = "post">
        @csrf
    ...
    <input id="blt" type="radio" name="jenis_bansos" class="hidden {{ $errors ->has('jenis_bansos') ? 'is-invalid' : ''}}" value = "BLT" checked />
   ...
   <div class="flex items-center mr-4 mb-4">
    <input id="bnpt" type="radio" name="jenis_bansos" class="hidden {{ $errors ->has('jenis_bansos') ? 'is-invalid' : ''}}" value = "BNPT" checked/>
