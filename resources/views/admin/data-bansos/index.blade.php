...
@section('content')
		...
		<h1 class="text-3xl py-4 border-b mb-10 text-white">Data Bansos</h1>
				...
				<tbody>
					...
					@foreach ($databansos as $data)
						<tr>
							...
							<td class="border border-slate-300 border-t border-gray-200 userId text-center">
							<a href="{{url('admin/data-bansos')}}/{{$data->id}}/{{'edit'}}" class='btn btn-success btn-sm'><i class='fas fa-pencil-alt'></i></a>
							<form action="{{url('admin/data-bansos')}}/{{$data->id}}" method = "post">
							@method('DELETE')
							@csrf
							<button type = "submit">
