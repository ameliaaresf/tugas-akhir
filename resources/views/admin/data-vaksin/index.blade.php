			...
			<div>
				<div class="">
                    <div class="rounded bg-green-500 hover:bg-green-700 py-2 px-4 text-white mr-5">
                        <a href="{{ route('admin.data-vaksin.create')}}">Tambah</a>
                    </div>
				...
				<tbody>
					...
					@foreach ($datavaksin as $data)
						<tr>
							...
							<td class="border border-slate-300 border-t border-gray-200 userId text-center">
							<a href="{{url('admin/data-vaksin')}}/{{$data->id}}/{{'edit'}}" class='btn btn-success btn-sm'><i class='fas fa-pencil-alt'></i></a>
							<form action="{{url('admin/data-vaksin')}}/{{$data->id}}" method = "post">
							@method('DELETE')
							@csrf
							<button type = "submit">
