...
<div class = "bg-gray-100 w-full mx-4 rounded-lg">
    ...
    <form class="w-full mt-10 px-4 rounded-lg" action="{{route('admin.sku.store')}}" method = "post">
        @csrf
    ...
    <label for="perempuan" class="flex items-center cursor-pointer">
     ...
     @if($errors->has('jk'))
        ...
        <div class="w-full px-3 mb-6 md:mb-0 my-4">
          ...
          <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 
          {{ $errors ->has('nama_usaha') ? 'is-invalid' : ''}}" id="nama_usaha" name = "nama_usaha" type="text" placeholder="Masukkan nama_usaha">
